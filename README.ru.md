...
трясёт руль на каждый выстрел из вашей машины, включая выстрелы союзников, а толчок, которого вы не
вызывали, читается как неисправность руля.
...
| Wheel weight | центрирующая пружина и демпфер - на месте и в движении |
...
2. **Привяжите кнопку режима A/M и выберите, как она себя ведёт.** Под ней две кнопки:
   ...
   Выберите ту, что совпадает с железом, которое вы привязали. Именно здесь чаще всего ошибаются:
   работают обе, а совпадает с движением вашей руки только одна.
...
Тоже действует сразу - перезапуск не нужен.
...
- **Клавиши подстройки камеры на ходу** опциональны и в этой версии **работают только с
  клавиатуры. Кнопки руля здесь не поддерживаются** - переназначить клавиши по-прежнему можно,
  а F1-F6 это раскладка по умолчанию.
...
оставить открытой рядом с Mafia и почувствовать изменение на следующем повороте. Исключение
одно: привязки H-шифтера читаются при старте игры.
